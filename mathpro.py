...
# Inyección de estilos CSS avanzados según el tema seleccionado
...
# --- API KEY DE GROQ GLOBAL ESTÁTICA ---
api_key = "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"

# Inicialización estricta de variables de control en session_state
...
# Títulos de la aplicación corporativa
...
with tab1:
    ...
    # --- MÓDULO DE CÁMARA CORREGIDO AL MODELO DE PRODUCCIÓN DE GROQ ---
    if activar_camara:
        ...
        if foto_archivo is not None:
            with st.spinner("Analizando caracteres con el motor de visión de producción..."):
                try:
                    client = Groq(api_key=api_key)
                    imagen_bytes = foto_archivo.read()
                    base64_image = base64.b64encode(imagen_bytes).decode('utf-8')
                    
                    # Forzamos el uso del modelo de visión de producción estándar con soporte activo
    ...
    # Interfaz de captura de parámetros matemáticos
    ...
    # Controles dinámicos según operación seleccionada
    lim_target = "0"
    orden_derivada = 1
    ...
    if st.button("Calcular Todo", type="primary", use_container_width=True):
        try:
            ...
            # --- SECCIÓN CRÍTICA: EL PROMPT DE CÁTEDRA UNIVERSITARIA ORIGINAL ---
            with st.spinner("Generando explicación académica detallada con Groq..."):
                try:
                    client = Groq(api_key=api_key)
                    prompt = f"""
                    Actúa como un prestigioso y riguroso profesor de matemáticas de nivel de Ingeniería de Sistemas.
                    Explica de forma elocuente, fluida y con un lenguaje matemático avanzado pero sumamente comprensible la resolución de la operación {operation} aplicada a la función o expresión ${sp.latex(f)}$.
                    El resultado exacto analítico es ${sp.latex(result) if not isinstance(result, str) else result}$.
                    No estructures la respuesta con títulos genéricos como "Paso 1", "Paso 2" ni listas rígidas de objetivos. En su lugar, dicta la explicación fluidamente como si estuvieras frente a la pizarra, explicando el porqué de cada transformación algebraica, propiedad de límites, o teorema de cálculo aplicado.
                    Utiliza exclusivamente la notación matemática de LaTeX encerrando las variables y ecuaciones entre símbolos de '$' para asegurar una visualización impecable.
                    """
    ...
    # Despliegue de los paneles informativos del cálculo
    if st.session_state.ultimo_calculo is not None:
        ...
        # Gestión y almacenamiento del reporte matemático
        ...
        # Renderizado de Gráficas en la base de la pestaña
